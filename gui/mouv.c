...
int deplace_coord(const coord_t * coord, SDL_Rect * position, const char labyrinthe[N_LAB][M_LAB], char dir, int vitesse) {
	switch(dir) {
		case 'h':
...
void deplace_fantome(fantome_t * fantome, char labyrinthe[N_LAB][M_LAB], SDL_Renderer * rend, joueur_t * joueur) {
	...
	if(fantome->position.x % TAILLE_CASE == 0 && fantome->position.y % TAILLE_CASE == 0) {
		...
		if(nb_chemins_voisins(labyrinthe, fantome->coord.x, fantome->coord.y) > 2 || (
			(fantome->dir == 'g' && !est_chemin(labyrinthe[fantome->coord.y][fantome->coord.x-1])) ||
			(fantome->dir == 'd' && !est_chemin(labyrinthe[fantome->coord.y][fantome->coord.x+1])) ||
			(fantome->dir == 'h' && !est_chemin(labyrinthe[fantome->coord.y-1][fantome->coord.x])) ||
			(fantome->dir == 'b' && !est_chemin(labyrinthe[fantome->coord.y+1][fantome->coord.x])))) {
			dest = fantome->chemin(labyrinthe, &(fantome->coord), &(joueur->coord));
	...
	deplace_coord(&(fantome->coord), &(fantome->position), labyrinthe, fantome->dir, 3);
